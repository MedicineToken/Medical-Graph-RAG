...
class Neo4jGraph(BaseGraphStorage):
    ...
    def _get_node_import_query(
        ...
        if base_entity_label:
            return (
                ...
                "UNWIND $data AS row "
                f"MERGE (source:`{BASE_ENTITY_LABEL}` {{id: row.id, gid: row.properties.gid}}) "
            ...
            return (
                f"{INCLUDE_DOCS_QUERY if include_source else ''}"
                "UNWIND $data AS row "
                "CALL apoc.merge.node([row.type], {id: row.id, gid: row.properties.gid}, row.properties, {}) "
                "YIELD node "
    ...
    def _get_rel_import_query(self, base_entity_label: bool) -> str:
        ...
        if base_entity_label:
            return (
                "UNWIND $data AS row "
                f"MERGE (subj:`{BASE_ENTITY_LABEL}` {{id: row.subj, gid: row.properties.gid}}) "
                f"MERGE (obj:`{BASE_ENTITY_LABEL}` {{id: row.obj, gid: row.properties.gid}}) "
                "WITH subj, obj, row "
            ...
            return (
                "UNWIND $data AS row "
                "CALL apoc.merge.node([row.subj_label], {id: row.subj, gid: row.properties.gid}, {}, {}) YIELD node as subj "
                "CALL apoc.merge.node([row.obj_label], {id: row.obj, gid: row.properties.gid}, {}, {}) YIELD node as obj "
                "CALL apoc.merge.relationship(subj, row.type, "
                "{}, row.properties, obj) YIELD rel "
                "RETURN distinct 'done'"
            )
